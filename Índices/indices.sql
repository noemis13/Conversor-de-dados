CREATE UNIQUE INDEX coordenadas ON wkt USING gin (known_text, id_wkt);

CREATE UNIQUE INDEX id_estado ON statefp(id_state, id_statefp);

CREATE UNIQUE INDEX codigo_fip ON countyfp(code_fip, id_countyfp);

CREATE UNIQUE INDEX nCasa_endereco_borda_esquerda ON lfromadd USING gin (number_house_adress_left_border, id_lfromadd);

CREATE UNIQUE INDEX nCasa_endereco_borda_esquerda ON ltoadd USING gin (number_house_adress_left_border, id_lfromadd);

CREATE UNIQUE INDEX nCasa_endereco_borda_direita ON rfromadd USING gin (number_house_adress_right_border, id_rfromadd);

CREATE UNIQUE INDEX nCasa_borda_direita ON rtomadd USING gin (number_house_right_border, id_rtoadd);

CREATE UNIQUE INDEX codigoZip_bordaEsquerda ON zipl(zip_code_left, id_zipl);

CREATE UNIQUE INDEX codigoZip_bordaDireita ON zipl(zip_code_right, id_zipr);
...
CREATE UNIQUE INDEX indicador_estrada_pavimentada ON ttyp(pack_type, id_ttyp);
...
CREATE UNIQUE INDEX indicador_caracteristicas_linear ON olfflg(linear_id, id_divroad, id_passflg, id_persistl, id_olfflg);
...
CREATE UNIQUE INDEX caracteristicas_arestas ON fullname USING gin (name_edges, id_fullname, id_wkt, id_statefp, id_countyfp, id_tlid, id_tfidl, id_tfidr, id_mtfcc, id_smid, id_lfromadd, id_ltoadd, id_rfromadd, id_rtoadd, id_zipl, id_zipr, id_hydroflg, id_railflg, id_roadflg, id_passflg, id_divroad, id_exttyp, id_deckedroad, id_artpath, id_persist, id_gcseflg, id_offsetl, id_offsetr, id_tnidf, id_tnidt);
